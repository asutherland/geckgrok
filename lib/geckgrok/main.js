...
LineReader.prototype = {
  readLine: function() {
    var s;
    var idxNewline = this.buf.indexOf("\n");
    if (idxNewline > -1) {
      s = this.buf.substring(0, idxNewline);
      this.buf = this.buf.substring(idxNewline+1);
      return s;
    }
    var read = this.stream.read(1024);
    if (!read.length) {
      if (this.buf.length) {
        s = this.buf;
        this.buf = "";
...
console.log("  linked!");
  ...
  "!!DOCSHELL": function(ctx, line) {
    ...
    if (presPtr) {
      ...
      docShell.presShells.push(presShell);
    }
  },
  /**
...
function closeOutDomWindow(ctx, aDomWindow) {
  var docShells = aDomWindow.docShells;
  console.log("closing out", aDomWindow.ptr, "it has", docShells.length);
  for (var iDocShell = 0; iDocShell < docShells.length; iDocShell++) {
    var docShell = docShells[iDocShell];
    var presShells = docShell.presShells;
    console.log("  doc shell", docShell.ptr, "has", presShells.length);
...
exports.main = function geckgrok_main(options, callbacks) {
  ...
  if (args === undefined) {
    ...
    return callbacks.quit("EMISUSE");
  }

  if (args.length == 0) {
    console.error("Specify a command.");
    return callbacks.quit("ENOCOMMAND");
  }

  if (args[0] == "splice") {
    console.log("starting geckgrok process");
    var fs = file.open("/tmp/framedebug", "r");
    //var ts = new textstreams.TextReader(fs);
    var lr = new LineReader(fs);
  ...
  return callbacks.quit("EBADCOMMAND");
};
